...
module.exports.showBytag = (req, res) =>{
  let tag = req.params.tagName.toLowerCase()

  Tweet.find({tags: {$in: [tag]}}).populate('author').sort('-data').limit(100)
    .then(tweets =>{
...
module.exports.like = (req, res) =>{
  ...
  Tweet.findById(id).then(tweet =>{
    ...
    tweet.save().then(tweet =>{
      User.findByIdAndUpdate(req.user._id, {$addToSet: {likedTweets: _id}}).then((user) =>{
        res.redirect('/')
      })
    })
  })
...
module.exports.dislike = (req, res) =>{
  ...
  Tweet.findById(id).then(tweet =>{
    tweet.likes = tweet.likes - 1
    tweet.save().then(tweet =>{
      User.findByIdAndUpdate(req.user._id, {$pull: {likedTweets: {$in: [tweet._id]}}})
        .then((user) =>{
        res.redirect('/')
        })
    })
